...
public class Mortgage {
    ...
    public static void main(String[] args) {
        try {
            ...
            // prompt the user for a double representing the annual interest rate. For
            // example, 3.5% would be entered as .035
            double annualIntrestRate = getAnnualIntrestRate();

            // prompt the user for the number of years the mortgage will be held (typical
            // input here is 10, 15, or 30)
            double yearsOfMortgage = getYearsOfMortgage();

            // prompt the user for a number representing the mortgage amount borrowed from
            // the bank
            ...
            // print the monthly payment in dollars, with only two significant digits after
            // the decimal point
            printMonthlyPayment(monthlyPayment);

            // print the total payment over the years, with only two significant digits
            // after the decimal point
            double total = (monthlyPayment * 12) * nOY;
            printTotal(total);

            // print the overpayment, i.e., the difference between the total payment over
            // the years and the mortgage amount
            // with only two significant digits after the decimal point
            double overPayment = total - m;
            printOverPayment(overPayment);

            // print the overpayment as a percentage (in percent notation) of the mortgage
            // amount
